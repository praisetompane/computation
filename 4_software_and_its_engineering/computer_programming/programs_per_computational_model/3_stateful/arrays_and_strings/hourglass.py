...
def hourglassSum(input_arr):
    hour_glass_sum_max = -9 * 7
    excluded_indices = 1
    max_index = len(input_arr[0]) - excluded_indices
    print(max_index)

    def sum_hour_glass(center):
        def sum_row(row, col):
            return (
                input_arr[row][col - 1] + input_arr[row][col] + input_arr[row][col + 1]
            )

        row = center[0]
        col = center[1]
        return input_arr[row][col] + sum_row(row - 1, col) + sum_row(row + 1, col)
